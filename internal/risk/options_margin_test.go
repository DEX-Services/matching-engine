...
func newOptionOrder(side models.OrderSide, symbol, optionType, strike string, expiry time.Time) *models.Order {
	return &models.Order{
		...
		StrikePrice: fixedpoint.MustFromString(strike), Expiry: expiry,
		QuoteCurrency: "BI2XUSD", Quantity: fixedpoint.FromInt64(1),
	}
}
...
func TestOptionsMargin_FallsBackToCashSecuredWithoutMarkSource(t *testing.T) {
	...
	order := newOptionOrder(models.Sell, "BTC-BI2XUSD-60000-20260101-CALL", "CALL", "60000", time.Now().Add(24*time.Hour))
	got := shortOptionMargin(order, fixedpoint.FromInt64(1), fixedpoint.FromInt64(500))
	want := fixedpoint.FromInt64(60000)
	if !got.Equal(want) {
...
func TestOptionsMargin_OTMCallUsesFloorBelowCashSecured(t *testing.T) {
	SetMarkSource(fakeMarkSource{"BTC-BI2XUSD": fixedpoint.FromInt64(50000)})
	...
	order := newOptionOrder(models.Sell, "BTC-BI2XUSD-60000-20260101-CALL", "CALL", "60000", time.Now().Add(24*time.Hour))
	got := shortOptionMargin(order, fixedpoint.FromInt64(1), fixedpoint.FromInt64(500))
	want := fixedpoint.FromInt64(10500)
	if !got.Equal(want) {
		t.Fatalf("shortOptionMargin OTM call = %s, want %s", got, want)
	}
	if got.GreaterThan(fixedpoint.FromInt64(60000)) {
...
func TestOptionsMargin_ITMPutUsesIntrinsicFloor(t *testing.T) {
	SetMarkSource(fakeMarkSource{"BTC-BI2XUSD": fixedpoint.FromInt64(40000)})
	t.Cleanup(func() { markSource = nil })
	...
	order := newOptionOrder(models.Sell, "BTC-BI2XUSD-60000-20260101-PUT", "PUT", "60000", time.Now().Add(24*time.Hour))
	got := shortOptionMargin(order, fixedpoint.FromInt64(1), fixedpoint.FromInt64(200))
	want := fixedpoint.FromInt64(20000)
	if !got.Equal(want) {
...
func TestOptionsMargin_NeverExceedsCashSecured(t *testing.T) {
	...
	SetMarkSource(fakeMarkSource{"BTC-BI2XUSD": fixedpoint.FromInt64(200000)})
	t.Cleanup(func() { markSource = nil })

	order := newOptionOrder(models.Sell, "BTC-BI2XUSD-60000-20260101-CALL", "CALL", "60000", time.Now().Add(24*time.Hour))
	got := shortOptionMargin(order, fixedpoint.FromInt64(1), fixedpoint.FromInt64(999999))
	cashSecured := fixedpoint.FromInt64(60000)
...
func TestOptionsMargin_BuyerAlwaysPaysPremiumRegardlessOfMarkSource(t *testing.T) {
	...
	order := newOptionOrder(models.Buy, "BTC-BI2XUSD-60000-20260101-CALL", "CALL", "60000", time.Now().Add(24*time.Hour))
	got := notionalFor(order, fixedpoint.FromInt64(2), fixedpoint.FromInt64(500))
	want := fixedpoint.FromInt64(1000) // premium * qty, untouched by the margin floor logic
	if !got.Equal(want) {
