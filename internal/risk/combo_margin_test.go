...
func TestAssetFor_ComboUsesQuoteCurrencyThenFirstLegSymbol(t *testing.T) {
	...
	if got := assetFor(withQuote); got != "BI2XUSD" {
		t.Fatalf("assetFor with QuoteCurrency set = %s, want BI2XUSD", got)
	}

	noQuote := comboOrder(models.Buy, verticalLegs(), "400")
	noQuote.QuoteCurrency = ""
	if got := assetFor(noQuote); got != "BI2XUSD" {
		t.Fatalf("assetFor falling back to first-leg symbol parse = %s, want BI2XUSD", got)
	}
}
