...
func TestForceClosePosition_UnknownPositionIsNoop(t *testing.T) {
	ledger := risk.NewLedger()
	os := NewOptionsSettlement(ledger, &backendclient.Client{})
	pnl, err := os.ForceClosePosition("nobody", "BTC-BI2XUSD-60000-20260101-CALL", decimal.NewFromInt(60000), time.Now().Add(time.Hour), "CALL", decimal.Zero, decimal.NewFromInt(100))
	if err != nil {
