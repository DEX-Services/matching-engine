...
func verticalSetup(t *testing.T) (*OptionsSettlement, *ComboSettlement, string, string, time.Time) {
	...
	ledger.Deposit("seller", "BI2XUSD", decimal.NewFromInt(1_000_000))
	...
	marks := fakeMarkSource{
		...
		legs: map[string]legSpec{
			buySymbol:  {strike: decimal.NewFromInt(60000), expiry: expiry, optionType: "CALL", quoteCurrency: "BI2XUSD"},
			sellSymbol: {strike: decimal.NewFromInt(65000), expiry: expiry, optionType: "CALL", quoteCurrency: "BI2XUSD"},
		},
	}
...
func TestComboSettle_IronCondorFansOutIntoFourLegPositions(t *testing.T) {
	...
	marks := fakeMarkSource{
		...
		legs: map[string]legSpec{
			longPut:   {strike: decimal.NewFromInt(50000), expiry: expiry, optionType: "PUT", quoteCurrency: "BI2XUSD"},
			shortPut:  {strike: decimal.NewFromInt(55000), expiry: expiry, optionType: "PUT", quoteCurrency: "BI2XUSD"},
			shortCall: {strike: decimal.NewFromInt(65000), expiry: expiry, optionType: "CALL", quoteCurrency: "BI2XUSD"},
			longCall:  {strike: decimal.NewFromInt(70000), expiry: expiry, optionType: "CALL", quoteCurrency: "BI2XUSD"},
		},
	}
	combo := NewComboSettlement(options, legs, marks)
...
func TestLegPricesInvariant_TwoLeg(t *testing.T) {
	...
	netPrice := decimal.NewFromInt(400)
	prices := splitComboNetPrice(netPrice, specs, "BTC-BI2XUSD", marks, decimal.NewFromFloat(0.03))
	if len(prices) != 2 {
